...
if (isGetCookie) {
    !(async() => {
        ...
        if ($.setdata(JSON.stringify(session), $.signKeySXMD)) { $.subt = `获取会话:成功!` } else { $.subt = `获取会话:失败!` }
        $.msg($.name, $.subt, '')
    })().catch((e) => $.logErr(e)).finally(() => $.done())
} else {!(async() => { await doTask(); if ($.ret) await doTask2() })().catch((e) => $.logErr(e)).finally(() => $.done()) }

function doTask() {
    return new Promise(resolve => {
        $.post(taskUrl(), (err, resp, data) => {
            try {
                if (err) { data = JSON.parse(resp.body) } else {
                    ...
                    if (!coin) {
                        ...
                        for (var i = 0; i < mm.length; i++) { $.subt = $.subt + mm[i] }
                        $.subt = $.subt + '！'
                    } else {
                        $.ret = 0;
                        $.subt = 'Cooies失效，请点击通知重新获取！';
                        var pictureURL1 = 'https://raw.githubusercontent.com/ybaojin/fun/main/Icon/sxmd.png';
...
function taskUrl2() { return { url: 'http://www.txtnovel.top/plugin.php?id=dsu_paulsign:sign&mobile=yes', headers: { 'Cookie': JSON.parse($.getdata($.signKeySXMD)).headers['Cookie'], 'User-Agent': 'Mozilla/5.0 (iPhone; CPU iPhone OS 15_4 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148 MicroMessenger/8.0.18(0x1800123c) NetType/WIFI Language/zh_TW' } } }

function taskUrl() { return { url: 'http://www.txtnovel.top/plugin.php?id=dsu_paulsign:sign&operation=qiandao&infloat=0&inajax=0&mobile=yes', body: JSON.parse($.getdata($.signKeySXMD)).body, headers: { 'Cookie': JSON.parse($.getdata($.signKeySXMD)).headers['Cookie'], 'User-Agent': 'Mozilla/5.0 (iPhone; CPU iPhone OS 15_4 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148 MicroMessenger/8.0.18(0x1800123c) NetType/WIFI Language/zh_TW' } } }
